...
import PIL.Image
import os
from Model_for_three import MobileV2
...
class HIL_regression(object):
    def __init__(self, regression_model_path):
        ...
        model = MobileV2()
        regression_model_path = './checkpoints/20000-mobilenet_net3.pt'
        model.load_state_dict(torch.load(
            regression_model_path, map_location=device))
        model.eval()
        img = np.random.rand(1, 3, 64, 64)
        img = img.astype(np.float32)
        img = torch.from_numpy(img)
        torch.onnx.export(model, img, 'test.onnx', verbose=False, opset_version=12,
